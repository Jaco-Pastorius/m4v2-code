...
def plot_robot(
    ...
    states_lables = [r"$x$",r"$y$",r"$z$", r"$\psi$", r"$ \theta $", r"$ \phi $", r"$\dot x$",r"$\dot y$", r"$\dot z$",r"$\omega_x$", r"$\omega_y$",r"$\omega_z$"]
...
def closed_loop_simulation():
    ...
    for i in range(Nsim):
        ...
        # get optimal state and input
        u_opt = acados_ocp_solver.get(0, "u")
        x_opt = acados_ocp_solver.get(1, "x")  

        simU[i,:] = u_opt

        # # get desired body rates
        # omega_d = np.array([x_opt[-3],x_opt[-2],x_opt[-1]])

        # # # apply body rate controller
        # # x = [xcurrent[9],xcurrent[10],xcurrent[11]]
        # # xd = omega_d
        # # ud = u_opt 
        ...
        # # print(f"xd: {xd}")
        # # print(f"ud: {ud}")
        # # u_filtered = body_rate_lqr(x,varphi_0,xd)
