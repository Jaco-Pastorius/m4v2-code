...
from morphing_lander.mpc.mpc import create_ocp_solver_description_hybrid
from morphing_lander.mpc.parameters import params_
from morphing_lander.mpc.trajectories import traj_jump_time
from morphing_lander.mpc.integrator import create_sim_solver_description_hybrid
...
def plot_robot(
    ...
    control_lables = [r"$u_1$", r"$u_2$",r"$u_3$", r"$u_4$", r"$u_5$", r"$u_6$"]
...
def closed_loop_simulation():
    ...
    ocp = create_ocp_solver_description_hybrid()
    sim = create_sim_solver_description_hybrid()
    ...
    for stage in range(N_horizon + 1):
        ...
        acados_ocp_solver.set(stage, "p", np.array([np.pi/2,True]))

    for stage in range(N_horizon):
        acados_ocp_solver.set(stage, "u", np.zeros(6))
    ...
    for i in range(Nsim):
        ...
        for j in range(N_horizon):
            ...
            acados_ocp_solver.set(j, "p", np.array([phicurrent,True]))
        ...
        acados_integrator.set("p",np.array([phicurrent,True]))
