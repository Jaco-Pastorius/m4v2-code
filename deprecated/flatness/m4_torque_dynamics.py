...
def m4_update(t, x_, u, params):
    ...
    varphi = params.get('varphi',  0.0)      # update the tilt angle
    J = np.zeros((3,3))
    J[0,0] = 0.002
    J[1,1] = 0.008
    J[2,2] = 0.007
    ...
    # get states
    x, y, z, dx, dy, dz, phi,th,psi,ox,oy,oz   = x_
    ...
    return np.array([dx,dy,dz,pdd[0],pdd[1],pdd[2],chid[2],chid[1],chid[0],od[0],od[1],od[2]])

# m4 output
def m4_output(t, x_, u, params):
    x, y, z, dx, dy, dz, phi,th,psi,ox,oy,oz   = x_
    return np.array([x, y, z, dx, dy, dz, phi,th,psi,ox,oy,oz])
